defmodule ZclassicExplorerWeb.VkLive do
  ...
  def render(assigns) do
    ...
    <td class="px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-500">
        <%= ZclassicExplorerWeb.AddressView.zatoshi_to_zec(tx["amount"]) %>
    </td>
    <td class="px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-500">
        <%= tx["address"] %>
    </td>
    <td class="px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-500">
        <%= ZclassicExplorerWeb.BlockView.mined_time_rel(tx["datetime"]) %>
    </td>
  ...
  def mount(_params, session, socket) do
    if connected?(socket) do
      ...
      Phoenix.PubSub.subscribe(ZclassicExplorer.PubSub, "VK:" <> "#{container_id}")
    end
