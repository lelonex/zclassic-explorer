defmodule ZclassicExplorerWeb.BlockController do
  ...
  def get_block(conn, %{"hash" => hash}) do
    {:ok, basic_block_data} = Zclassicex.getblock(hash, 1)
    ...
    case length(basic_block_data["tx"]) do
      ...
      n when n <= 250 ->
        {:ok, block_data} = Zclassicex.getblock(hash, 2)
        block_data = Zclassicex.Block.from_map(block_data)
  ...
  def index(conn, %{"date" => date}) do
    ...
    case Zclassicex.getblockhashes(high, low, true, false) do
      {:ok, blocks} ->
        blocks_data =
          blocks
          |> Task.async_stream(fn block -> Zclassicex.getblockheader(block) end,
