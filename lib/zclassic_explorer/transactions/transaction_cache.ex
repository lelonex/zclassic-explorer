defmodule ZclassicExplorer.Transactions.TransactionWarmer do
  ...
  def execute(_state) do
    ...
    case Zclassicex.getblockhashes(high, low, true, true) do
      {:ok, blocks} ->
        blocks
        |> Enum.sort(&(&1["logicalts"] >= &2["logicalts"]))
        |> Enum.map(fn x ->
          {:ok, block} = Zclassicex.getblock(Map.get(x, "blockhash"), 1)
          tx = block["tx"]
        end)
        |> List.flatten()
        |> Enum.take(10)
        |> Enum.map(fn y ->
          {:ok, tx} = Zclassicex.getrawtransaction(y, 1)
        ...
        |> Enum.map(fn z ->
          %{
            ...
            "time" => ZclassicExplorerWeb.BlockView.mined_time(Map.get(z, :time)),
            "tx_out_total" => ZclassicExplorerWeb.BlockView.tx_out_total(z),
            "size" => Map.get(z, :size),
            "type" => ZclassicExplorerWeb.BlockView.tx_type(z)
          }
        end)
        |> handle_result
